  ...
  <div class="min-h-screen bg-dark-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <nav class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <a href="/products" class="hover:text-electric transition-colors">Products</a>
        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"/></svg>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-6">
              ...
              <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
                ...
                <div class="bg-electric/5 border border-electric/10 rounded-2xl p-5 min-w-[260px]">
                  <div class="flex items-center justify-between mb-3">
                    <span class="text-xs font-semibold tracking-widest uppercase text-electric">New Order</span>
                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber/20 text-amber uppercase tracking-wider">Draft</span>
                  </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <input
                    ...
                    class="w-full bg-dark-800 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-electric/50 focus:ring-1 focus:ring-electric/25 transition-all"
                    required
                  />
                ...
                <div>
                  <label class="block text-sm text-gray-400 mb-2">Jumlah</label>
                  <div class="flex items-center gap-3">
                    <button
                      type="button"
                      @click="decrementQty"
                      class="w-11 h-11 flex items-center justify-center bg-dark-800 border border-white/10 rounded-xl text-white hover:bg-white/5 transition-all"
                    >
                    ...
                    <input
                      ...
                      min="1"
                      :max="product.stock"
                      class="w-16 text-center bg-dark-800 border border-white/10 rounded-xl px-2 py-3 text-white focus:outline-none focus:border-electric/50 transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <button
                      type="button"
                      @click="incrementQty"
                    ...
                    <span class="text-xs text-gray-500 ml-1">Stok: {{ product.stock }}</span>
                  </div>
                ...
                <div class="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    class="w-full bg-dark-800 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-electric/50 focus:ring-1 focus:ring-electric/25 transition-all resize-none"
                    required
                  ></textarea>
            ...
            <div class="glass-card rounded-3xl p-8">
              ...
              <!-- Table Header -->
              <div class="hidden sm:grid grid-cols-12 gap-4 text-xs text-gray-500 uppercase tracking-wider font-semibold pb-3 border-b border-white/5 mb-4">
              ...
              <!-- Product Row -->
              <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center py-4 border-b border-white/5">
                <div class="sm:col-span-5 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-gray-500 text-xs">{{ product.category ? product.category.name : 'Uncategorized' }}</p>
                  </div>
                </div>
                <div class="sm:col-span-2 text-center text-gray-300 text-sm">
                  <span class="sm:hidden text-gray-500">Qty: </span>{{ form.quantity }}
                </div>
                <div class="sm:col-span-2 text-right text-gray-300 text-sm">
                  <span class="sm:hidden text-gray-500">Harga: </span>Rp {{ formatPrice(product.price) }}
                </div>
                <div class="sm:col-span-3 text-right text-white font-semibold text-sm">
                  <span class="sm:hidden text-gray-500">Total: </span>Rp {{ formatPrice(subtotal) }}
                </div>
              </div>
              ...
              <div class="mt-4 p-4 bg-dark-800/50 rounded-xl">
                ...
                <p class="text-sm text-gray-400">{{ product.description || 'Tidak ada deskripsi produk.' }}</p>
              </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-8">
              ...
              <div class="grid grid-cols-2 gap-3 mb-6">
                <button
                  ...
                  :class="[
                    ...
                    form.payment_method === 'bank_transfer'
                      ...
                      : 'border-white/5 hover:border-white/15 bg-dark-800'
                  ]"
                >
                  <div class="flex items-center gap-3">
                    <div :class="['w-10 h-10 rounded-xl flex items-center justify-center', form.payment_method === 'bank_transfer' ? 'bg-electric/20' : 'bg-white/5']">
                      <svg class="w-5 h-5" :class="form.payment_method === 'bank_transfer' ? 'text-electric' : 'text-gray-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  type="button"
                  @click="form.payment_method = 'qris'; form.payment_bank = null"
                  :class="[
                    'p-4 rounded-2xl border-2 transition-all duration-300 text-left',
                ...
                >
                  <div class="flex items-center gap-3">
                    <div :class="['w-10 h-10 rounded-xl flex items-center justify-center', form.payment_method === 'qris' ? 'bg-neon/20' : 'bg-white/5']">
                      <svg class="w-5 h-5" :class="form.payment_method === 'qris' ? 'text-neon' : 'text-gray-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <!-- Bank Selector (shown when bank_transfer selected) -->
              <transition
                ...
                <div v-if="form.payment_method === 'bank_transfer'" class="space-y-3">
                  ...
                  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    <button
                      ...
                      :key="bank.code"
                      type="button"
                      @click="form.payment_bank = bank.code"
                      :class="[
                        'p-4 rounded-xl border-2 transition-all duration-300 flex flex-col items-center gap-2',
                        form.payment_bank === bank.code
                          ? 'border-electric bg-electric/5'
                          : 'border-white/5 hover:border-white/15 bg-dark-800'
                      ]"
                    >
                      <div class="w-10 h-10 rounded-lg flex items-center justify-center text-lg font-bold"
                           :class="form.payment_bank === bank.code ? 'text-electric' : 'text-gray-500'"
                           :style="{ backgroundColor: form.payment_bank === bank.code ? bank.color + '15' : 'rgba(255,255,255,0.03)' }">
                        {{ bank.shortName }}
                      </div>
                      <span :class="['text-xs font-medium', form.payment_bank === bank.code ? 'text-white' : 'text-gray-500']">{{ bank.name }}</span>
                    </button>
                  </div>
                  <p v-if="errors.payment_bank" class="text-red-400 text-xs mt-2">{{ errors.payment_bank }}</p>
                </div>
              </transition>
              <p v-if="errors.payment_method" class="text-red-400 text-xs mt-2">{{ errors.payment_method }}</p>
          ...
          <div class="lg:col-span-1">
            <div class="sticky top-6 space-y-6">
              ...
              <div class="glass-card rounded-3xl p-7">
                ...
                <div v-if="form.payment_method" class="p-4 bg-dark-800/50 rounded-xl mb-6">
                  <p class="text-xs text-gray-500 mb-2">Metode Pembayaran</p>
                  <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-lg bg-electric/10 flex items-center justify-center">
                      <svg v-if="form.payment_method === 'bank_transfer'" class="w-4 h-4 text-electric" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                      </svg>
                      <svg v-else class="w-4 h-4 text-neon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
                      </svg>
                    </div>
                    <div>
                      <p class="text-white text-sm font-medium">
                        {{ form.payment_method === 'bank_transfer' ? 'Bank Transfer' : 'QRIS' }}
                      </p>
                      <p v-if="form.payment_bank" class="text-gray-500 text-xs">{{ selectedBankName }}</p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  :class="[
                    'w-full py-4 rounded-2xl font-semibold text-white transition-all duration-300 flex items-center justify-center gap-2',
                    isSubmitting
                      ? 'bg-gray-700 cursor-not-allowed'
                      : 'bg-gradient-to-r from-electric to-neon hover:shadow-lg hover:shadow-electric/25 transform hover:scale-[1.02] active:scale-[0.98]'
                  ]"
                >
...
const subtotal = computed(() => props.product.price * form.quantity);
const tax = computed(() => subtotal.value * 0.11);
const totalAmount = computed(() => subtotal.value + tax.value);

const selectedBankName = computed(() => {
  const bank = banks.find(b => b.code === form.payment_bank);
